...
def split_large_chunk(chunk: dict, max_tokens: int = MAX_TOKENS) -> list:
        """Chia chunk qua lon theo khoan/doan van"""
...
def process_all(clean_dir: Path, output_dir: Path):
    ...
    for md_file in sorted(clean_dir.glob("**/*.md")):
        ...
        # Phan loai: tieu chuan nuoc ngoai hay van ban tieng Viet?
        ...
        # Guard: neu khong parse duoc chunk nao thi bo qua, khong crash
        if not merged:
